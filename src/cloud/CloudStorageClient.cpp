...
nlohmann::json CloudStorageClient::loadJson()
{
...
void CloudStorageClient::saveJson(const nlohmann::json& data)
{
...
void CloudStorageClient::pushRawContent(const std::string& content) const
{
    CURL* curl = curl_easy_init();
    if (curl == nullptr)
        throw CloudStorageException("Nie można zainicjalizować CURL");
